...
[CanEditMultipleObjects]
    ...
    private string ReadStringFormFile(string path)
    {
        if (File.Exists(path))
        {
    ...
    public static GUIStyle CloneGUIStyle(GUIStyle obj)
        ...
        protected static GUIStyle DescLabelStyle
        {
            get
            {
                ...
                {
                    _descLabelStyle = CloneGUIStyle(GUI.skin.GetStyle("Label"));
        ...
        private void _shadercacheInit(Shader shader)
        {
            ...
            _shaderCahceCode = ReadStringFormFile(path);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (targetMat != null)
            {
                ...
                {
                    if (targetMat.GetFloat("_BloomFactor") <= 0)
                    {
        ...
        {
            string path = Application.dataPath.Replace("Assets", "") + _shaderPath;
            if (SaveStringToFile(path, _shaderCahceCode))
            {
    ...
    public bool SaveStringToFile(string path, string fileStr)
    {
