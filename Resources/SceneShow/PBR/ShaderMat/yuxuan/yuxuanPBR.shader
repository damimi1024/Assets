...
Shader "yuxuan/PBR1"
{
    Properties
    {
        ...
        _SmoothnessTex("Roughness(R)",2D) = "white"{}
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i): SV_Target
            {
                //贴图采样
                    ...
                    fixed4 MetallicSampler = tex2D(_MetallicTex,i.uv);          //采样Metallic贴图
                    fixed4 SmoothnessSampler = tex2D(_SmoothnessTex,i.uv);      //采样Smoothness贴图
                    fixed4 AlbedoColorSampler = tex2D(_MainTex, i.uv) * _Color; //采样颜色贴图，同时乘以控制的TintColor
                    fixed4 Occlusion = tex2D(_OcclussionTex,i.uv)*_AO;          //赋予AO

                //方向向量
                    
                    half3 worldNormal = half3(dot(i.tSpace0,normalTex),dot(i.tSpace1,normalTex),dot(i.tSpace2,normalTex));  //切线空间法线（带贴图）转向世界空间法线。
                    worldNormal = normalize(worldNormal);                                                                   //所有传入的“向量”最好归一化一下
                    fixed3 lightDir = normalize(UnityWorldSpaceLightDir(i.worldPos));                                       //计算灯光方向：注意这个方法已经包含了对灯光的判定。其实在forwardbase pass中，可以直接用灯光坐标代替这个方法，因为只会计算Directional Light。
                ...
                o.Smoothness = smoothness;                          //a通道乘以控制色并赋予光滑度
